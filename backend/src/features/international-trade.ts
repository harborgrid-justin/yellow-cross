/**
 * Feature 51: International Trade Management
...
const createSchema = Joi.object({
  matterType: Joi.string().valid('customs', 'export-controls', 'sanctions', 'tariffs', 'trade-agreement', 'dispute', 'other').required(),
  companyName: Joi.string().required(),
  countries: Joi.string().optional(),
  commodities: Joi.string().optional(),
...
const updateSchema = Joi.object({
  status: Joi.string().valid('active', 'review', 'clearance', 'investigation', 'appeal', 'resolved', 'closed').optional(),
  notes: Joi.string().optional(),
  ...
  try {
    if (!isConnected()) {
      return res.status(200).json({
        feature: 'International Trade Management - Create',
    ...
    const matterNumber = `ITR-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`.toUpperCase();

    const matter = await InternationalTradeMatter.create({
      matterNumber,
      ...validatedData,
      status: 'active'
    });
...
router.get('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'International Trade Management - Details', capabilities: ['View details', 'Document access'] });
    }

    const matter = await InternationalTradeMatter.findByPk(req.params.id);
    if (!matter) {
...
router.put('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'International Trade Management - Update', capabilities: ['Update records', 'Status changes'] });
    }
...
router.get('/', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'International Trade Management - List', capabilities: ['List all', 'Filter', 'Search', 'Sort'] });
    }

    const matters = await InternationalTradeMatter.findAll({
...
router.delete('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'International Trade Management - Delete', capabilities: ['Delete matter', 'Archive'] });
    }
