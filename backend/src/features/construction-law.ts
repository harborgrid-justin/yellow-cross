  ...
  try {
    if (!isConnected()) {
      return res.status(200).json({
        ...
        description: 'Construction disputes',
...
router.get('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Construction Law Management - Details', capabilities: ['View details', 'Document access', 'History tracking'] });
    }
...
router.delete('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Construction Law Management - Delete', capabilities: ['Soft delete', 'Archive'] });
    }
