/**
 * Feature 54: Civil Rights Management
...
const createSchema = Joi.object({
  matterType: Joi.string().valid('discrimination', 'police-misconduct', 'voting-rights', 'housing-discrimination', 'constitutional-violation', 'other').required(),
  plaintiffName: Joi.string().required(),
  defendantName: Joi.string().optional(),
...
const updateSchema = Joi.object({
  status: Joi.string().valid('active', 'investigation', 'complaint-filed', 'discovery', 'mediation', 'trial', 'appeal', 'closed').optional(),
  notes: Joi.string().optional(),
  ...
  try {
    if (!isConnected()) {
      return res.status(200).json({
        feature: 'Civil Rights Management - Create',
    ...
    const matterNumber = `CIV-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`.toUpperCase();

    const matter = await CivilRightsMatter.create({
      matterNumber,
      ...validatedData,
      status: 'active'
    });
...
router.get('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Civil Rights Management - Details', capabilities: ['View details', 'Document access'] });
    }

    const matter = await CivilRightsMatter.findByPk(req.params.id);
    if (!matter) {
...
router.put('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Civil Rights Management - Update', capabilities: ['Update records', 'Status changes'] });
    }
...
router.get('/', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Civil Rights Management - List', capabilities: ['List all', 'Filter', 'Search', 'Sort'] });
    }

    const matters = await CivilRightsMatter.findAll({
...
router.delete('/:id', async (req, res) => {
  try {
    if (!isConnected()) {
      return res.status(200).json({ feature: 'Civil Rights Management - Delete', capabilities: ['Delete matter', 'Archive'] });
    }
