/**
 * AppellatePracticeService
 * Business logic for AppellatePractice
 * 
...
export class AppellatePracticeService extends BaseService<AppellateCase> {
   ...
   */
  async findByStatus(status: string): Promise<AppellateCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<AppellateCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
...
export default AppellatePracticeService;
