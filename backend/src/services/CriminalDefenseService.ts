/**
 * CriminalDefenseService
 * Business logic for CriminalDefense
 * 
...
export class CriminalDefenseService extends BaseService<CriminalDefenseCase> {
  constructor() {
    super(CriminalDefenseCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<CriminalDefenseCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<CriminalDefenseCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<CriminalDefenseCase | null> {
...
export default CriminalDefenseService;
