/**
 * FranchiseLawService
 * Business logic for FranchiseLaw
 * 
...
export class FranchiseLawService extends BaseService<FranchiseLawMatter> {
  constructor() {
    super(FranchiseLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<FranchiseLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<FranchiseLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<FranchiseLawMatter | null> {
...
export default FranchiseLawService;
