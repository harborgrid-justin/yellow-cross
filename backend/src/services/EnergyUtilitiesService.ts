/**
 * EnergyUtilitiesService
 * Business logic for EnergyUtilities
 * 
...
export class EnergyUtilitiesService extends BaseService<EnergyUtilitiesMatter> {
  constructor() {
    super(EnergyUtilitiesMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<EnergyUtilitiesMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<EnergyUtilitiesMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<EnergyUtilitiesMatter | null> {
...
export default EnergyUtilitiesService;
