/**
 * EnvironmentalLawService
 * Business logic for EnvironmentalLaw
 * 
...
export class EnvironmentalLawService extends BaseService<EnvironmentalLawMatter> {
  constructor() {
    super(EnvironmentalLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<EnvironmentalLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<EnvironmentalLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<EnvironmentalLawMatter | null> {
...
export default EnvironmentalLawService;
