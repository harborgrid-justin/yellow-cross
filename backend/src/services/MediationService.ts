...
import { MediationCase } from '../models/sequelize/MediationCase';

export class MediationService extends BaseService<MediationCase> {
  constructor() {
    super(MediationCase);
  }
   ...
   */
  async findByStatus(status: string): Promise<MediationCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<MediationCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
