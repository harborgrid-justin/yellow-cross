/**
 * CybersecurityLegalService
 * Business logic for CybersecurityLegal
 * 
...
export class CybersecurityLegalService extends BaseService<CybersecurityLegalMatter> {
  constructor() {
    super(CybersecurityLegalMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<CybersecurityLegalMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<CybersecurityLegalMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<CybersecurityLegalMatter | null> {
...
export default CybersecurityLegalService;
