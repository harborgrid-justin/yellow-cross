/**
 * SportsEntertainmentService
 * Business logic for SportsEntertainment
 * 
...
export class SportsEntertainmentService extends BaseService<SportsEntertainmentMatter> {
  constructor() {
    super(SportsEntertainmentMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<SportsEntertainmentMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<SportsEntertainmentMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<SportsEntertainmentMatter | null> {
...
export default SportsEntertainmentService;
