 ...
 * Business logic for Communication
 * 
...
import { CommunicationTemplate } from '../models/sequelize/CommunicationTemplate';

export class CommunicationService extends BaseService<CommunicationTemplate> {
  constructor() {
    super(CommunicationTemplate);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<CommunicationTemplate[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<CommunicationTemplate[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
