/**
 * EducationLawService
 * Business logic for EducationLaw
 * 
...
export class EducationLawService extends BaseService<EducationLawMatter> {
  constructor() {
    super(EducationLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<EducationLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<EducationLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<EducationLawMatter | null> {
...
export default EducationLawService;
