 ...
 * Business logic for ImmigrationLaw
 * 
...
export class ImmigrationLawService extends BaseService<ImmigrationCase> {
  constructor() {
    super(ImmigrationCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<ImmigrationCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ImmigrationCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
...
export default ImmigrationLawService;
