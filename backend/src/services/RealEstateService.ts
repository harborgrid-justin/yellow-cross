...
export class RealEstateService extends BaseService<RealEstateTransaction> {
  constructor() {
    super(RealEstateTransaction);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<RealEstateTransaction[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<RealEstateTransaction[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<RealEstateTransaction | null> {
