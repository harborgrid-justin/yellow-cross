/**
 * NonProfitLawService
 * Business logic for NonProfitLaw
 * 
...
export class NonProfitLawService extends BaseService<NonProfitLawMatter> {
  constructor() {
    super(NonProfitLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<NonProfitLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<NonProfitLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<NonProfitLawMatter | null> {
...
export default NonProfitLawService;
