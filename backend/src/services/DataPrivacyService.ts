/**
 * DataPrivacyService
 * Business logic for DataPrivacy
 * 
...
export class DataPrivacyService extends BaseService<DataPrivacyMatter> {
  constructor() {
    super(DataPrivacyMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<DataPrivacyMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<DataPrivacyMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<DataPrivacyMatter | null> {
