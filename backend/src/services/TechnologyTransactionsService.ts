/**
 * TechnologyTransactionsService
 * Business logic for TechnologyTransactions
 * 
...
export class TechnologyTransactionsService extends BaseService<TechnologyTransactionsMatter> {
  constructor() {
    super(TechnologyTransactionsMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<TechnologyTransactionsMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<TechnologyTransactionsMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<TechnologyTransactionsMatter | null> {
...
export default TechnologyTransactionsService;
