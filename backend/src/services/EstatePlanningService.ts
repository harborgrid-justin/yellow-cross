 ...
 * Business logic for EstatePlanning
 * 
...
export class EstatePlanningService extends BaseService<EstatePlanningMatter> {
  constructor() {
    super(EstatePlanningMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<EstatePlanningMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<EstatePlanningMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<EstatePlanningMatter | null> {
