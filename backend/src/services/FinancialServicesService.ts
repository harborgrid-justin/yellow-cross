...
export class FinancialServicesService extends BaseService<FinancialServicesMatter> {
  constructor() {
    super(FinancialServicesMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<FinancialServicesMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<FinancialServicesMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<FinancialServicesMatter | null> {
...
export default FinancialServicesService;
