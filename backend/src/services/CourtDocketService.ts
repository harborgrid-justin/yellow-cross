/**
 * CourtDocketService
 * Business logic for CourtDocket
 * 
...
export class CourtDocketService extends BaseService<CourtDocket> {
  constructor() {
    super(CourtDocket);
  }
   ...
   */
  async findByStatus(status: string): Promise<CourtDocket[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<CourtDocket[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
...
export default CourtDocketService;
