/**
 * AviationLawService
 * Business logic for AviationLaw
 * 
...
export class AviationLawService extends BaseService<AviationLawMatter> {
  constructor() {
    super(AviationLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<AviationLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<AviationLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<AviationLawMatter | null> {
...
export default AviationLawService;
