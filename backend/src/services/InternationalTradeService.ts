 ...
 * Business logic for InternationalTrade
 * 
...
export class InternationalTradeService extends BaseService<InternationalTradeMatter> {
  constructor() {
    super(InternationalTradeMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<InternationalTradeMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<InternationalTradeMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<InternationalTradeMatter | null> {
