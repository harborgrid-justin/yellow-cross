...
import { IntegrationConfig } from '../models/sequelize/IntegrationConfig';

export class IntegrationService extends BaseService<IntegrationConfig> {
  constructor() {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<IntegrationConfig[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
