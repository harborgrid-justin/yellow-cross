/**
 * ConstructionLawService
 * Business logic for ConstructionLaw
 * 
...
export class ConstructionLawService extends BaseService<ConstructionLawMatter> {
  constructor() {
    super(ConstructionLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<ConstructionLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ConstructionLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<ConstructionLawMatter | null> {
...
export default ConstructionLawService;
