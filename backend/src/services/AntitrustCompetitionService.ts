/**
 * AntitrustCompetitionService
 * Business logic for AntitrustCompetition
 * 
...
export class AntitrustCompetitionService extends BaseService<AntitrustCompetitionMatter> {
  constructor() {
    super(AntitrustCompetitionMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<AntitrustCompetitionMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<AntitrustCompetitionMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<AntitrustCompetitionMatter | null> {
...
export default AntitrustCompetitionService;
