/**
 * VeteransAffairsService
 * Business logic for VeteransAffairs
 * 
...
export class VeteransAffairsService extends BaseService<VeteransAffairsMatter> {
  constructor() {
    super(VeteransAffairsMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<VeteransAffairsMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<VeteransAffairsMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<VeteransAffairsMatter | null> {
...
export default VeteransAffairsService;
