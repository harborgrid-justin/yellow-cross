/**
 * SecuritiesLawService
 * Business logic for SecuritiesLaw
 * 
...
export class SecuritiesLawService extends BaseService<SecuritiesLawMatter> {
  constructor() {
    super(SecuritiesLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<SecuritiesLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<SecuritiesLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<SecuritiesLawMatter | null> {
...
export default SecuritiesLawService;
