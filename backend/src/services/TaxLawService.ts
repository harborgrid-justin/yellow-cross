...
export class TaxLawService extends BaseService<TaxLawMatter> {
   ...
   */
  async findByStatus(status: string): Promise<TaxLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<TaxLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
