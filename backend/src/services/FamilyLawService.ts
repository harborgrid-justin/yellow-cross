...
export class FamilyLawService extends BaseService<FamilyLawCase> {
   ...
   */
  async findByStatus(status: string): Promise<FamilyLawCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<FamilyLawCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
