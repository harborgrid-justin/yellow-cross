/**
 * PersonalInjuryService
 * Business logic for PersonalInjury
 * 
...
export class PersonalInjuryService extends BaseService<PersonalInjuryCase> {
  constructor() {
    super(PersonalInjuryCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<PersonalInjuryCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<PersonalInjuryCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<PersonalInjuryCase | null> {
...
export default PersonalInjuryService;
