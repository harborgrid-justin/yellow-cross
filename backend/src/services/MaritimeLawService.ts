/**
 * MaritimeLawService
 * Business logic for MaritimeLaw
 * 
...
export class MaritimeLawService extends BaseService<MaritimeLawMatter> {
  constructor() {
    super(MaritimeLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<MaritimeLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<MaritimeLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<MaritimeLawMatter | null> {
...
export default MaritimeLawService;
