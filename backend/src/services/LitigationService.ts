...
import { LitigationCase } from '../models/sequelize/LitigationCase';

export class LitigationService extends BaseService<LitigationCase> {
  constructor() {
    super(LitigationCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<LitigationCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<LitigationCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
