/**
 * TimeBillingService
 * Business logic for TimeBilling
 * 
...
export class TimeBillingService extends BaseService<TimeEntry> {
  constructor() {
    super(TimeEntry);
  }
   ...
   */
  async findByStatus(status: string): Promise<TimeEntry[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<TimeEntry[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
...
export default TimeBillingService;
