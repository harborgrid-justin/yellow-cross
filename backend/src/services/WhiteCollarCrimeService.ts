/**
 * WhiteCollarCrimeService
 * Business logic for WhiteCollarCrime
 * 
...
export class WhiteCollarCrimeService extends BaseService<WhiteCollarCrimeMatter> {
  constructor() {
    super(WhiteCollarCrimeMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<WhiteCollarCrimeMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<WhiteCollarCrimeMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<WhiteCollarCrimeMatter | null> {
...
export default WhiteCollarCrimeService;
