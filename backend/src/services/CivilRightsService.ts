/**
 * CivilRightsService
 * Business logic for CivilRights
 * 
...
export class CivilRightsService extends BaseService<CivilRightsMatter> {
  constructor() {
    super(CivilRightsMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<CivilRightsMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<CivilRightsMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<CivilRightsMatter | null> {
...
export default CivilRightsService;
