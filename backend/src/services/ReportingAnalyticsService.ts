 ...
 * Business logic for ReportingAnalytics
 * 
...
export class ReportingAnalyticsService extends BaseService<Report> {
  constructor() {
    super(Report);
  }
   ...
   */
  async findByAssignee(assignedTo: string): Promise<Report[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
