 ...
 * Business logic for SocialSecurity
 * 
...
export class SocialSecurityService extends BaseService<SocialSecurityMatter> {
  constructor() {
    super(SocialSecurityMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<SocialSecurityMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<SocialSecurityMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<SocialSecurityMatter | null> {
