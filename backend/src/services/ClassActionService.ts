/**
 * ClassActionService
 * Business logic for ClassAction
 * 
...
export class ClassActionService extends BaseService<ClassActionCase> {
  constructor() {
    super(ClassActionCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<ClassActionCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ClassActionCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
...
export default ClassActionService;
