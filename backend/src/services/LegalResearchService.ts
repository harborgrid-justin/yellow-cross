 ...
 * Business logic for LegalResearch
 * 
...
export class LegalResearchService extends BaseService<ResearchItem> {
  constructor() {
    super(ResearchItem);
  }
   ...
   */
  async findByStatus(status: string): Promise<ResearchItem[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ResearchItem[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
