/**
 * HealthcareLawService
 * Business logic for HealthcareLaw
 * 
...
export class HealthcareLawService extends BaseService<HealthcareLawMatter> {
  constructor() {
    super(HealthcareLawMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<HealthcareLawMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<HealthcareLawMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<HealthcareLawMatter | null> {
...
export default HealthcareLawService;
