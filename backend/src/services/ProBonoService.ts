...
export class ProBonoService extends BaseService<ProBonoMatter> {
   ...
   */
  async findByStatus(status: string): Promise<ProBonoMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ProBonoMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
