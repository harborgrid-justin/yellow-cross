 ...
 * Business logic for LandlordTenant
 * 
...
export class LandlordTenantService extends BaseService<LandlordTenantMatter> {
  constructor() {
    super(LandlordTenantMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<LandlordTenantMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<LandlordTenantMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<LandlordTenantMatter | null> {
...
export default LandlordTenantService;
