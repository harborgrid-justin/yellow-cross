 ...
 * Business logic for IntellectualProperty
 * 
...
import { IntellectualPropertyMatter } from '../models/sequelize/IntellectualPropertyMatter';

export class IntellectualPropertyService extends BaseService<IntellectualPropertyMatter> {
  constructor() {
    super(IntellectualPropertyMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<IntellectualPropertyMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<IntellectualPropertyMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<IntellectualPropertyMatter | null> {
