 ...
 * Business logic for ConsumerProtection
 * 
...
export class ConsumerProtectionService extends BaseService<ConsumerProtectionMatter> {
  constructor() {
    super(ConsumerProtectionMatter);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<ConsumerProtectionMatter[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<ConsumerProtectionMatter[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
  ...
  async updateStatus(
    ...
    status: string,
    updatedBy: string
  ): Promise<ConsumerProtectionMatter | null> {
