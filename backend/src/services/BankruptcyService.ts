...
export class BankruptcyService extends BaseService<BankruptcyCase> {
  constructor() {
    super(BankruptcyCase);
  }

  /**
   * Find records by status
   */
  async findByStatus(status: string): Promise<BankruptcyCase[]> {
   ...
   */
  async findByAssignee(assignedTo: string): Promise<BankruptcyCase[]> {
    return await this.findAll({
      where: { assignedTo },
      order: [['createdAt', 'DESC']]
    });
