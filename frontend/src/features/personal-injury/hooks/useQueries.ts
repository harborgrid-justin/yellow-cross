...
export function usePersonalInjurys(params?: PaginationParams) {
  ...
  return useQuery<PaginatedResponse<PersonalInjury>>(`/personalinjury${queryParams.toString() ? `?${queryParams.toString()}` : ''}`);
}

export function usePersonalInjury(id: string, options?: { skip?: boolean }) {
  return useQuery<PersonalInjury>(`/personalinjury/${id}`, options);
}
