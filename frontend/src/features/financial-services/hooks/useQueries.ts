...
export function useFinancials(params?: PaginationParams) {
  ...
  return useQuery<PaginatedResponse<Financial>>(`/financial-services${queryParams.toString() ? `?${queryParams.toString()}` : ''}`);
}

export function useFinancial(id: string, options?: { skip?: boolean }) {
  return useQuery<Financial>(`/financial-services/${id}`, options);
}
