/**
 * WF-COMP-XXX | reportingAnalyticsApi.ts - Reporting Analytics API service module
 * Purpose: Reporting Analytics domain API operations with type safety and validation
 * Upstream: ../config/apiConfig, ../utils/apiUtils, ../../shared/types | Dependencies: axios, zod
 * Downstream: Components, Redux stores | Called by: Reporting Analytics components and stores
 * Related: Report types, reportingAnalytics Redux slice
 * Exports: reportingAnalyticsApi instance, types | Key Features: CRUD operations, validation, error handling
...
export type ReportStatus = 'Open' | 'In Progress' | 'Closed' | 'On Hold' | 'Pending Review' | 'Archived';
...
export interface ReportingAnalyticsApi {
  // Basic CRUD operations
  getAll(filters?: ReportFilters): Promise<PaginatedResponse<Report>>;
  ...
  // Advanced operations
  getStatistics(filters?: ReportFilters): Promise<ReportStatistics>;
  search(query: string, filters?: ReportFilters): Promise<Report[]>;
}
...
/**
 * Update Report validation schema (partial of create)
 */
 ...
 */
const reportingAnalyticsFiltersSchema = z.object({
...
class ReportingAnalyticsApiImpl implements ReportingAnalyticsApi {
  private readonly baseEndpoint = '/api/reporting-analytics';
  private readonly auditResource = 'REPORT' as const;

  /**
   * Get all Report items with filtering and pagination
   */
  async getAll(filters?: ReportFilters): Promise<PaginatedResponse<Report>> {
    try {
      // Validate filters
      const validatedFilters = filters ? reportingAnalyticsFiltersSchema.parse(filters) : {};
      ...
      const data = extractApiData<PaginatedResponse<Report>>(response);
  ...
  async create(data: CreateReportData): Promise<Report> {
    try {
      ...
      const created = extractApiData<Report>(response);
  ...
  async update(id: string, data: UpdateReportData): Promise<Report> {
    try {
      ...
      const updated = extractApiData<Report>(response);
  ...
  /**
   * Get statistics for Report items
   */
  async getStatistics(filters?: ReportFilters): Promise<ReportStatistics> {
    try {
      ...
      const data = extractApiData<ReportStatistics>(response);
   ...
   * Search Report items
   */
  async search(query: string, filters?: ReportFilters): Promise<Report[]> {
    try {
...
/**
 * Singleton instance of the Reporting Analytics API
 * @example
 * import { reportingAnalyticsApi } from '@/services';
 * 
 * // Get all items
 * const items = await reportingAnalyticsApi.getAll({ status: 'Open' });
 * 
 * // Create new item
 * const newItem = await reportingAnalyticsApi.create({
 *   title: 'New Item',
 *   status: 'Open',
 *   priority: 'High'
 * });
 */
export const reportingAnalyticsApi: ReportingAnalyticsApi = new ReportingAnalyticsApiImpl();
