/**
 * WF-COMP-XXX | contractManagementApi.ts - Contract Management API service module
 * Purpose: Contract Management domain API operations with type safety and validation
 * Upstream: ../config/apiConfig, ../utils/apiUtils, ../../shared/types | Dependencies: axios, zod
 * Downstream: Components, Redux stores | Called by: Contract Management components and stores
 * Related: Contract types, contractManagement Redux slice
 * Exports: contractManagementApi instance, types | Key Features: CRUD operations, validation, error handling
...
export type ContractStatus = 'Open' | 'In Progress' | 'Closed' | 'On Hold' | 'Pending Review' | 'Archived';
...
export interface ContractManagementApi {
  // Basic CRUD operations
  getAll(filters?: ContractFilters): Promise<PaginatedResponse<Contract>>;
  ...
  // Advanced operations
  getStatistics(filters?: ContractFilters): Promise<ContractStatistics>;
  search(query: string, filters?: ContractFilters): Promise<Contract[]>;
}
...
/**
 * Update Contract validation schema (partial of create)
 */
 ...
 */
const contractManagementFiltersSchema = z.object({
...
class ContractManagementApiImpl implements ContractManagementApi {
  private readonly baseEndpoint = '/api/contract-management';
  private readonly auditResource = 'CONTRACT' as const;

  /**
   * Get all Contract items with filtering and pagination
   */
  async getAll(filters?: ContractFilters): Promise<PaginatedResponse<Contract>> {
    try {
      // Validate filters
      const validatedFilters = filters ? contractManagementFiltersSchema.parse(filters) : {};
      ...
      const data = extractApiData<PaginatedResponse<Contract>>(response);
  ...
  async getById(id: string): Promise<Contract> {
    try {
      ...
      const data = extractApiData<Contract>(response);
  ...
  async create(data: CreateContractData): Promise<Contract> {
    try {
      ...
      const created = extractApiData<Contract>(response);
  ...
  async update(id: string, data: UpdateContractData): Promise<Contract> {
    try {
      ...
      const updated = extractApiData<Contract>(response);
  ...
  /**
   * Get statistics for Contract items
   */
  async getStatistics(filters?: ContractFilters): Promise<ContractStatistics> {
    try {
      const validatedFilters = filters ? contractManagementFiltersSchema.parse(filters) : {};
      const params = buildUrlParams(validatedFilters);
      ...
      const data = extractApiData<ContractStatistics>(response);
   ...
   */
  async search(query: string, filters?: ContractFilters): Promise<Contract[]> {
    try {
...
/**
 * Singleton instance of the Contract Management API
 * @example
 * import { contractManagementApi } from '@/services';
 * 
 * // Get all items
 * const items = await contractManagementApi.getAll({ status: 'Open' });
 * 
 * // Create new item
 * const newItem = await contractManagementApi.create({
 *   title: 'New Item',
 *   status: 'Open',
 *   priority: 'High'
 * });
 */
export const contractManagementApi: ContractManagementApi = new ContractManagementApiImpl();
